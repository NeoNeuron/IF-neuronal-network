...
def process_main(path, spike_id, lfp_name, delay_range, time_range, dt):
    ...
    #print('./bin/spike.out', path + '/raster.csv', spike_name, str(spike_id), time_range, str(dt), 'false')
...
for res in result:
  ...
  i += 1
np.savetxt(args.dir + "/mi_lfp_scan.csv", mi_scan_res, delimiter = ',')
print('[-] Output to file -> ' + args.dir + 'mi_lfp_scan.csv.')
finish = time.time()
