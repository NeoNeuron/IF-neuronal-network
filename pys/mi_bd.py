import numpy as np
import pandas as pd
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import subprocess
import sys
dt = 0.5
# Calculating Mutual information:
ntd = 30
ptd = 30
drange = str(ntd) + ',' + str(ptd)
binsize = sys.argv[1]
program = './bin/mi_bd.out'
subprocess.call([program, './data/tmp/singleSpike.bin', './data/tmp/singleI.bin', drange, str(dt), binsize])
data = pd.read_csv('data/mi/mi_bd.csv')
subprocess.call([program, './data/tmp/singleSpike_shuffle.bin', './data/tmp/singleI.bin', drange, str(dt), binsize])
data_shuffle = pd.read_csv('data/mi/mi_bd.csv')
# Plotting:
plotway = 'linear'
if plotway == 'linear':
  plt.plot(data['timelag']*dt, data['mi'], label = 'mi')
  plt.plot(data_shuffle['timelag']*dt, data_shuffle['mi'], label = 'mi_shuffle')
  ax = plt.gca()
  ax.yaxis.get_major_formatter().set_powerlimits((0,1))
elif plotway == 'log':
  plt.semilogy(data['timelag']*dt, data['mi'], label = 'mi')
  plt.semilogy(data_shuffle['timelag']*dt, data_shuffle['mi'], label = 'mi_shuffle')

plt.xlabel('Delay Time(ms)')
plt.ylabel('mutual info')
plt.legend()
plt.savefig('mi')
plt.close()
