...
if (indx + ptd > totaln - 1): ptd = totaln - 1 - indx
...
subprocess.call(['./bin/mi_bd.out', tmp_dir + 'singleSpike.csv', tmp_dir + 'singleI.csv', str(indx), str(ntd) + ',' + str(ptd), str(dt), '500'])
# import data of mi_bd.csv
mi = pd.read_csv('./data/mi/mi_bd.csv')

# excute mutual info calculation for base level;
subprocess.call(['./bin/mi_bd.out', tmp_dir + 'singleSpike_shuffle.csv', tmp_dir + 'singleI.csv', str(indx), str(ntd) + ',' + str(ptd), str(dt), '500'])
# import data of mi_bd.csv
