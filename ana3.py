...
for i in range(NN):
	...
	for i in range(num_of_trials):
		print i
		num_exc, num_inh = mylib.DivideNeuronalTypes(neuron_types = types, neuron_list = trials[i])
		data_out.loc[i, 'connecting portion'] = num_exc * 1.0 / num_in_lfp
		rate = 0
	...
	data_out.to_csv("./tdmi/figure-eps/ana1.csv", sep = '\t', float_format = '%.6f', index  = False, columns = ['connecting portion', 'mean firing rate', 'signal noise ratio', 'peak time', 'time constant'])
	os.system('mkdir -p ./tdmi/figure-eps/Apr24/t6-' + str(target_index) + '/tdmi-' + str(num_in_lfp) + '/')
	os.system('mv tdmi/figure-eps/*.csv tdmi/figure-eps/Apr24/t6-' + str(target_index) + '/tdmi-' + str(num_in_lfp) + '/')
	os.system('mv tdmi/figure-eps/*.eps tdmi/figure-eps/Apr24/t6-' + str(target_index) + '/tdmi-' + str(num_in_lfp) + '/')
