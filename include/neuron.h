...
private:
	...
	double excitatory_conductance_tmp_; // temporal storage of excitatory conductance;

	double inhibitory_conductance_; // Stored inhibitory conductivity;
	double inhibitory_conductance_tmp_; // temporal storage of inhibitory conductance;
	...
	void GenerateInternalPoisson(double tmax, bool outSet);

	// Input external Poisson sequence within each time step, autosort after generatation;
	void InputExternalPoisson(double tmax, vector<double> & x);
	...
	//		DOUBLE dt: size of time step, unit millisecond;
	//	Return: spike time if fires, otherwise -1;
	double CheckFire(double voltage, double dt);
...
public:
	...
	Neuron() {
		...
		g_m_ = 5.0e-2;
		tau_ = 2.0;
		resting_potential_ = 0.0;
		threshold_potential_ = 1.0;
		excitatory_reversal_potential_ = 14.0 / 3.0;
		inhibitory_reversal_potential_ = -2.0 / 3.0;
	...
	void SetFeedforwardStrength(double F);

	//	Set Poisson Rate: homogeneous Poisson driving rate of internal driving;
	void SetPoissonRate(double rate);
	...
	double UpdateNeuronalState(double t, double dt, vector<double> & inPE);
	...
	//					if not, return -1;
	double TemporallyUpdateNeuronalState(double t, double dt, vector<double> & inPE);
